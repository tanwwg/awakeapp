...
//  wakeapp
//
//  Created by Tan Thor Jen on 29/1/24.
//

import SwiftUI

@Observable class KeystrokeApp {
    ...
    func start() {
        ...
        DispatchQueue.global().async { [self] in
            let task = Process()
            task.arguments = []
            task.executableURL = URL(fileURLWithPath: "/Applications/GeForceNOW.app/Contents/MacOS/GeForceNOW")
    ...
    func stop() {
//        for app in NSWorkspace.shared.runningApplications.filter({ $0.bundleIdentifier == "com.nvidia.gfnpc.mall" }) {
//            let ret = app.terminate()
//            print("terminate \(app.processIdentifier) \(ret)")
//        }
    ...
    func findPid(bundleId: String) -> pid_t? {
        return NSWorkspace.shared.runningApplications
            .first(where: { app in app.bundleIdentifier == bundleId })?.processIdentifier
    }
    
    func post(event: CGEvent, bundleId: String) {
        guard let pid = findPid(bundleId: bundleId) else {
            print("Unable to find \(bundleId)")
            return
        }
        event.postToPid(pid)
        print("posted to \(bundleId)")
    }
...
//            "com.nvidia.nvcontainer",
        ...
        for p in procs {
            post(event: down, bundleId: p)
        }
        self.count += 1

        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [self] in
            for p in procs {
                post(event: up, bundleId: p)
            }
        }
